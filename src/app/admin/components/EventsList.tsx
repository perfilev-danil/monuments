...
export default function EventsList() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    form.append("beginning", formData.beginning);
    ...
    try {
      ...
      setFormData({ beginning: "", end: "", name: "", link: "" });
  ...
  const handleEditClick = () => {
    const event = events.find((m: any) => m.id === selectedId);
    if (!event) return;

    setFormData({
      beginning: event?.time_span?.beginning || "",
      end: event?.time_span?.end || "",
      name: event?.appellation_event?.value || "",
      link: event?.information_object_event?.value || "",
    });
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setEvents(events.map((m: any) => (m.id === selectedId ? updated : m)));
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      setEvents(events.filter((m: any) => m.id !== id));
      setSelectedId(null);
    } catch (error: any) {
      console.error(error.message);
    } finally {
