...
export default function DocumentsList() {
  ...
  const handleEditClick = () => {
    const document = documents.find((m: any) => m.id === selectedId);
    if (!document) return;

    setFormData({
      name: document?.value || "",
      link: document?.information_object_document?.value || "",
    });
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/admin/documents/${selectedId}`, {
        method: "PUT",
        body: form,
      });
      ...
      setDocuments(
        documents.map((m: any) => (m.id === selectedId ? updated : m))
      );
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      setDocuments(documents.filter((m: any) => m.id !== id));
      setSelectedId(null);
    } catch (error: any) {
      console.error(error.message);
    } finally {
      ...
      {showFormModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-lg w-full relative">
            ...
            <h2 className="text-xl font-semibold mb-4">Новый документ</h2>
          ...
          {documents.map((document: any) => (
            <tr
              ...
              className={`cursor-pointer ${
                selectedId === document.id ? "bg-blue-100" : ""
              }`}
            >
