...
export default function TechniquesList() {
  ...
  const handleEditClick = () => {
    const technique = techniques.find((m: any) => m.id === selectedId);
    if (!technique) return;
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/admin/techniques/${selectedId}`, {
        method: "PUT",
        body: form,
      });
      ...
      setTechniques(
        techniques.map((m: any) => (m.id === selectedId ? updated : m))
      );
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      setTechniques(techniques.filter((m: any) => m.id !== id));
      setSelectedId(null);
    } catch (error: any) {
      console.error(error.message);
    } finally {
      ...
      {showFormModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-lg w-full relative">
            ...
            <h2 className="text-xl font-semibold mb-4">Новая техника</h2>
          ...
          {techniques.map((technique: any) => (
            <tr
              ...
              className={`cursor-pointer ${
                selectedId === technique?.id ? "bg-blue-100" : ""
              }`}
            >
