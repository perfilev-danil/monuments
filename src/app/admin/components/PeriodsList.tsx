...
export default function PeriodsList() {
  ...
  const handleEditClick = () => {
    const period = periods.find((m: any) => m.id === selectedId);
    if (!period) return;
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/admin/periods/${selectedId}`, {
        method: "PUT",
        body: form,
      });
      ...
      setPeriods(periods.map((m: any) => (m.id === selectedId ? updated : m)));
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      setPeriods(periods.filter((m: any) => m.id !== id));
      setSelectedId(null);
    } catch (error: any) {
      console.error(error.message);
    } finally {
      ...
      {showFormModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-lg w-full relative">
            ...
            <h2 className="text-xl font-semibold mb-4">Новый период</h2>
          ...
          {periods.map((period: any) => (
            <tr
              ...
              className={`cursor-pointer ${
                selectedId === period?.id ? "bg-blue-100" : ""
              }`}
            >
              <td className="border border-gray-300 px-4 py-2">{period?.id}</td>
