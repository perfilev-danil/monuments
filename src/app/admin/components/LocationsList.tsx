...
export default function LocationsList() {
  ...
  const handleEditClick = () => {
    const location = locations.find((m: any) => m.id === selectedId);
    if (!location) return;
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/admin/locations/${selectedId}`, {
        method: "PUT",
        body: form,
      });
      ...
      setLocations(
        locations.map((m: any) => (m.id === selectedId ? updated : m))
      );
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      setLocations(locations.filter((m: any) => m.id !== id));
      setSelectedId(null);
    } catch (error: any) {
      console.error(error.message);
    } finally {
            ...
            {locations.map((location: any) => (
              <tr
                ...
                className={`cursor-pointer ${
                  selectedId === location?.id ? "bg-blue-100" : ""
                }`}
              >
