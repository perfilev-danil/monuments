...
export default function RolesList() {
  ...
  const handleEditClick = () => {
    const role = roles.find((m: any) => m.id === selectedId);
    if (!role) return;
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/admin/roles/${selectedId}`, {
        method: "PUT",
        body: form,
      });
      ...
      setRoles(roles.map((m: any) => (m.id === selectedId ? updated : m)));
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      setRoles(roles.filter((m: any) => m.id !== id));
      setSelectedId(null);
    } catch (error: any) {
      console.error(error.message);
    } finally {
      ...
      {showFormModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-lg w-full relative">
            ...
            <h2 className="text-xl font-semibold mb-4">Новая роль</h2>
          ...
          {roles.map((role: any) => (
            <tr
              ...
              className={`cursor-pointer ${
                selectedId === role?.id ? "bg-blue-100" : ""
              }`}
            >
